	...
	protected override void ResolveInt(SketchResolveParams parms)
	{
		...
		for (int i = 0; i < rects.Count; i++)
		{
			...
			if (Rand.Bool)
			{
				...
				{
					if (!valueOrDefault || parms.sketch.AnyTerrainAt(new IntVec3(j, 0, newZ)))
					{
						sketch.AddThing(ThingDefOf.Column, new IntVec3(j, 0, newZ), Rot4.North, stuff, 1, null, null);
					}
				}
			...
			else
			{
				...
				{
					if (!valueOrDefault || parms.sketch.AnyTerrainAt(new IntVec3(newX, 0, k)))
					{
						sketch.AddThing(ThingDefOf.Column, new IntVec3(newX, 0, k), Rot4.North, stuff, 1, null, null);
					}
				}
