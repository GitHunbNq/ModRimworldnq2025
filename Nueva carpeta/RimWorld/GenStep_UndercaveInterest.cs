...
public class GenStep_UndercaveInterest : GenStep
{
	...
	private void GenerateMushroomPatch(Map map, IntVec3 cell)
	{
		...
		foreach (IntVec3 item in GridShapeMaker.IrregularLump(cell, map, PatchSizeRange.RandomInRange))
		{
			...
			if (GenSpawn.CanSpawnAt(thingDef, item, map, null))
			{
				map.terrainGrid.SetTerrain(item, TerrainDefOf.SoilRich);
				Thing thing = ThingMaker.MakeThing(thingDef);
				if (Rand.Chance(0.7f) && GenPlace.TryPlaceThing(thing, item, map, ThingPlaceMode.Direct, null, null, null) && thing is Plant plant)
				{
	...
	{
		if (!GenPlace.TryPlaceThing(ThingMaker.MakeThing(ThingDefOf.AncientGenerator ?? ThingDefOf.ChemfuelPoweredGenerator), cell, map, ThingPlaceMode.Direct, null, null, null))
		{
			return;
		}
	...
	private void GenerateCorpseGear(Map map, IntVec3 cell)
	{
		...
		GenPlace.TryPlaceThing(thing, cell, map, ThingPlaceMode.Radius, null, null, null);
	}
	...
	private void GenerateSleepingFleshbeasts(Map map, IntVec3 cell)
	{
		...
		for (int i = 0; i < randomInRange; i++)
		{
			Pawn thing = PawnGenerator.GeneratePawn(PawnKindDefOf.Fingerspike, Faction.OfEntities, null);
