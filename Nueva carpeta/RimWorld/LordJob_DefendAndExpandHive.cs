...
public class LordJob_DefendAndExpandHive : LordJob
{
	...
	public override StateGraph CreateGraph()
	{
		...
		LordToil lordToil = new LordToil_AssaultColony();
		if (base.Map.generatorDef == MapGeneratorDefOf.InsectLair)
		{
			lordToil = new LordToil_NestAssault();
		}
		stateGraph.AddToil(lordToil);
		Transition transition = new Transition(lordToil_DefendAndExpandHive, aggressive ? lordToil : lordToil_DefendHiveAggressively);
		transition.AddTrigger(new Trigger_PawnHarmed(0.5f, requireInstigatorWithFaction: true, null, null, null));
		...
		Transition transition2 = new Transition(lordToil_DefendAndExpandHive, lordToil);
		transition2.AddTrigger(new Trigger_PawnHarmed(0.5f, requireInstigatorWithFaction: false, base.Map.ParentFaction, null, null));
		transition2.AddPostAction(new TransitionAction_EndAllJobs());
		stateGraph.AddTransition(transition2);
		Transition transition3 = new Transition(lordToil_DefendHiveAggressively, lordToil);
		transition3.AddTrigger(new Trigger_PawnHarmed(0.5f, requireInstigatorWithFaction: false, base.Map.ParentFaction, null, null));
		...
		Transition transition6 = new Transition(lordToil, lordToil_DefendAndExpandHive);
	...
	public override bool ValidateAttackTarget(Pawn searcher, Thing target)
	{
		...
		if (mentalStateDef != null && mentalStateDef.blocksDefendAndExpandHive)
		{
			return false;
		}
		return true;
