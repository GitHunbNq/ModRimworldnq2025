...
public class GenStep_InsectLairCave : GenStep
{
	...
	public override void Generate(Map map, GenStepParams parms)
	{
		...
		foreach (IntVec3 allCell2 in map.AllCells)
		{
			if (Rand.Chance(0.02f) && allCell2.GetTerrain(map) == TerrainDefOf.InsectSludge)
			{
				...
				GenPlace.TryPlaceThing(thing, allCell2, map, ThingPlaceMode.Near, null, null, null);
			}
		}
	...
	private static void GenerateRandomBurrowWalls(Map map)
	{
		...
		foreach (IntVec3 allCell in map.AllCells)
		{
			...
			if (allCell.GetFirstBuilding(map) == null && value > 0.7f && GenSpawn.CanSpawnAt(ThingDefOf.BurrowWall, allCell, map, null))
			{
				Thing thing = ThingMaker.MakeThing(ThingDefOf.BurrowWall);
				if (GenPlace.TryPlaceThing(thing, allCell, map, ThingPlaceMode.Direct, null, null, null))
				{
					thing.SetFaction(Faction.OfInsects);
				}
			}
	...
	private void GenerateRandomHives(Map map, int count)
	{
		...
		{
			if (CellFinder.TryFindRandomCell(map, (IntVec3 c) => GenSpawn.CanSpawnAt(ThingDefOf.Hive, c, map, null) && !c.InHorDistOf(exit.Position, 20f), out var result))
			{
	...
	private void GeneratePOIWalls(IntVec3 loc, Map map, out List<IntVec3> shape)
	{
		...
		foreach (IntVec3 item5 in shape)
		{
			...
			{
				Thing thing = ThingMaker.MakeThing(ThingDefOf.BurrowWall);
				if (GenPlace.TryPlaceThing(thing, item5, map, ThingPlaceMode.Direct, null, null, null))
				{
					thing.SetFaction(Faction.OfInsects);
				}
			}
	...
	private void GenerateBossRoom(IntVec3 loc, Map map)
	{
		...
		Pawn pawn = PawnGenerator.GeneratePawn(PawnKindDefOf.HiveQueen, Faction.OfInsects, null);
		...
		{
			if (CellFinder.TryRandomClosewalkCellNear(loc, map, 5, out var result2, (IntVec3 c) => GenSpawn.CanSpawnAt(ThingDefOf.Hive, c, map, null)))
			{
		...
		if (Entrance != null && Entrance.spawnGravcore)
		{
			...
			while (num4 > 0)
			{
				...
				GenPlace.TryPlaceThing(thing, loc, map, ThingPlaceMode.Near, null, null, null);
			}
		}
		else
		{
	...
	private void GenerateInsectHivePOI(IntVec3 loc, Map map)
	{
		...
		for (int i = 0; i < num; i++)
		{
			if (CellFinder.TryRandomClosewalkCellNear(loc, map, 5, out var result, (IntVec3 c) => GenSpawn.CanSpawnAt(ThingDefOf.Hive, c, map, null)))
			{
