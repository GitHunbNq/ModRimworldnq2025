...
public class Dialog_BillConfig : Window
{
	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		else if (bill.repeatMode == BillRepeatModeDefOf.TargetCount)
		{
			...
			listing_Standard2.Label(text, -1f, (TipSignal?)null);
		...
		listing_Standard5.Label(text4, -1f, (TipSignal?)null);
		Text.Font = GameFont.Small;
		...
		if (bill.recipe.products.Count == 1)
		{
			...
			Widgets.InfoCardButton(num2, rect3.y, thingDef3, GenStuff.DefaultStuffFor(thingDef3));
			num2 += 28f;
		}
		if (Widgets.ButtonImage(new Rect(num2, rect3.y, 30f, 30f), TexButton.Rename))
		{
	...
	protected virtual void DoIngredientConfigPane(float x, ref float y, float width, float height)
	{
		...
		listing_Standard.Label(text + ": " + text2, -1f, (TipSignal?)null);
