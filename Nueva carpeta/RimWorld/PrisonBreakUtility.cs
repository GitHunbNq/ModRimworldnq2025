...
public static class PrisonBreakUtility
{
	...
	public static bool TryFindGroupUpLoc(List<Pawn> escapingPrisoners, IntVec3 exitPoint, out IntVec3 groupUpLoc)
	{
		...
		using (PawnPath pawnPath = map.pathFinder.FindPathNow(escapingPrisoners[0].Position, exitPoint, TraverseParms.For(escapingPrisoners[0], Danger.Deadly, TraverseMode.PassDoors), null))
		{
			if (!pawnPath.Found)
			{
	...
	private static bool RoomsAreCloseToEachOther(Room a, Room b)
	{
		...
		using PawnPath pawnPath = a.Map.pathFinder.FindPathNow(anyCell, anyCell2, TraverseParms.For(TraverseMode.PassDoors), null);
		if (!pawnPath.Found)
		{
			return false;
		}
