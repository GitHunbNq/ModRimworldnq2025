...
public class JobGiver_RevenantEscape : ThinkNode_JobGiver
{
	protected override Job TryGiveJob(Pawn pawn)
	{
		...
		using (PawnPath pawnPath = pawn.Map.pathFinder.FindPathNow(pawn.Position, intVec, TraverseParms.For(pawn, Danger.Deadly, TraverseMode.PassDoors), null))
		{
			if (!pawnPath.Found)
			{
				using PawnPath path = pawn.Map.pathFinder.FindPathNow(pawn.Position, intVec, TraverseParms.For(pawn, Danger.Deadly, TraverseMode.PassAllDestroyableThings), null);
				IntVec3 cellBefore;
