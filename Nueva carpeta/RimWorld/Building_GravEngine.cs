	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		yield return new Command_Action
		{
			...
			action = delegate
			{
				...
				{
					Widgets.MouseAttachedLabel("ChooseWhoShouldInspect".Translate(this), 0f, 0f, null);
				});
			}
		};
		...
		static bool ValidateInspectionTarget(LocalTargetInfo targ)
		{
			...
			if (!pawn2.IsColonistPlayerControlled)
			{
				return false;
			}
			if (!pawn2.CanReach(pawn2.Position, PathEndMode.Touch, Danger.Deadly))
			{
				return false;
			}
			if (pawn2.Downed || !pawn2.health.capacities.CapableOf(PawnCapacityDefOf.Moving))
			{
				return false;
			}
			return true;
