...
public class ComplexThreatWorker_CryptosleepPods : ComplexThreatWorker
{
	...
	{
		if (base.CanResolveInt(parms) && ComplexUtility.TryFindRandomSpawnCell(ThingDefOf.AncientCryptosleepPod, parms.room, parms.map, out var _, 1, null) && parms.points >= PawnKindDefOf.AncientSoldier.combatPower)
		{
	...
	private List<Thing> SpawnCasketsWithHostiles(LayoutRoom room, float threatPoints, string openSignal, Map map)
	{
		...
		for (int i = 0; i < num; i++)
		{
			if (!ComplexUtility.TryFindRandomSpawnCell(ThingDefOf.AncientCryptosleepPod, room, map, out var spawnPosition, 1, null))
			{
				break;
			}
