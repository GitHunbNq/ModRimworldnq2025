...
public static class RCellFinder
{
	...
	{
		IntVec3 result2;
		Predicate<Region> validator = (Region r) => r.Room.PsychologicallyOutdoors && !r.IsForbiddenEntirely(searcher) && r.TryFindRandomCellInRegionUnforbidden(searcher, Validator, out result2);
		...
		return CellFinder.RandomRegionNear(result3, 14, traverseParms, validator, searcher).TryFindRandomCellInRegionUnforbidden(searcher, Validator, out result);
	...
	public static bool TryFindDirectFleeDestination(IntVec3 root, float dist, Pawn pawn, out IntVec3 result)
	{
		...
		for (int j = 0; j < 30; j++)
		{
			...
			using PawnPath path = pawn.Map.pathFinder.FindPathNow(pawn.Position, randomCell, pawn, null);
