			...
			action = delegate
			{
				...
				}, (LocalTargetInfo target) => ValidateSealer(target).Accepted, null, null, SealHatchIcon.Texture, playSoundOnAction: true, delegate(LocalTargetInfo target)
				{
					...
					if (pawn2 != null && pawn2.IsColonistPlayerControlled && !acceptanceReport.Accepted)
					{
						...
						{
							Widgets.MouseAttachedLabel(("CannotChooseSealer".Translate() + ": " + acceptanceReport.Reason.CapitalizeFirst()).Colorize(ColorLibrary.RedReadable), 0f, 0f, null);
						}
						else
						{
							Widgets.MouseAttachedLabel("CannotChooseSealer".Translate(), 0f, 0f, null);
						}
					}
				});
			}
