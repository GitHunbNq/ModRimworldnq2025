...
public class Dialog_StylingStation : Window
{
	...
	private void DrawPawn(Rect rect)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			RenderTexture image = PortraitsCache.Get(pawn, new Vector2(position.width, position.height), new Rot4(2 - i), PortraitOffset, 1.1f, supersample: true, compensateForUIScale: true, showHeadgear, showClothes, apparelColors, desiredHairColor, stylingStation: true, null);
	...
	private void DrawTabs(Rect rect)
	{
		...
		case StylingTab.Hair:
			...
			DrawStylingItemType(rect, ref hairScrollPosition, delegate(Rect r, HairDef h)
			{
				...
				Widgets.DefIcon(r, h, null, 1.25f, null, drawPlaceholder: false, null, null, null);
				GUI.color = Color.white;
			}, delegate(HairDef h)
			{
		...
		case StylingTab.Beard:
			...
			{
				GUI.color = desiredHairColor;
				Widgets.DefIcon(r, b, null, 1.25f, null, drawPlaceholder: false, null, null, null);
				GUI.color = Color.white;
			}, delegate(BeardDef b)
			{
		...
		case StylingTab.TattooFace:
			...
			{
				Widgets.DefIcon(r, t, null, 1f, null, drawPlaceholder: false, null, null, null);
	...
	{
		Widgets.ThingIcon(new Rect(rect.x, curY, Text.LineHeight, Text.LineHeight), ThingDefOf.Dye, null, null, 1.1f, null, null);
	...
	private void DrawApparelColor(Rect rect)
	{
		...
		foreach (Apparel item in pawn.apparel.WornApparel)
		{
			...
			{
				Widgets.ColorSelector(rect2, ref value, AllColors, out var _, Widgets.GetIconFor(item.def, item.Stuff, item.StyleDef, null), 22, 2, ColorSelecterExtraOnGUI);
				float num2 = rect2.x;
				if (pawn.Ideo != null && !Find.IdeoManager.classicMode)
				{
					rect2 = new Rect(num2, curY, 200f, 24f);
					if (Widgets.ButtonText(rect2, "SetIdeoColor".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
					{
				...
				{
					rect2 = new Rect(num2, curY, 200f, 24f);
					if (Widgets.ButtonText(rect2, "SetFavoriteColor".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
					{
	...
	private void DrawStylingItemType<T>(Rect rect, ref Vector2 scrollPosition, Action<Rect, T> drawAction, Action<T> selectAction, Func<StyleItemDef, bool> hasStyleItem, Func<StyleItemDef, bool> hadStyleItem, Func<StyleItemDef, bool> extraValidator = null, bool doColors = false) where T : StyleItemDef
	{
		...
		int num5 = 0;
		tmpStyleItems.Clear();
		tmpStyleItems.AddRange(DefDatabase<T>.AllDefs.Where((T x) => (devEditMode || PawnStyleItemChooser.WantsToUseStyle(pawn, x, null) || hadStyleItem(x)) && (extraValidator == null || extraValidator(x))));
	...
	{
		if (Widgets.ButtonText(new Rect(inRect.x, inRect.yMax - ButSize.y, ButSize.x, ButSize.y), "Cancel".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			Reset();
			Close();
		}
		if (Widgets.ButtonText(new Rect(inRect.xMin + inRect.width / 2f - ButSize.x / 2f, inRect.yMax - ButSize.y, ButSize.x, ButSize.y), "Reset".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			Reset();
			SoundDefOf.Tick_Low.PlayOneShotOnCamera();
		}
		if (!Widgets.ButtonText(new Rect(inRect.xMax - ButSize.x, inRect.yMax - ButSize.y, ButSize.x, ButSize.y), "Accept".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			return;
		}
