...
public sealed class PlaySettings : IExposable
{
	...
	private void DoWorldViewControls(WidgetRow row)
	{
		...
		if (row.ButtonIcon(TexButton.SearchButton, tooltip, null, null, null) || (KeyBindingDefOf.OpenMapSearch.JustPressed && Event.current.type == EventType.KeyDown))
		{
	...
	private void DoMapControls(WidgetRow row)
	{
		...
		if (row.ButtonIcon(TexButton.SearchButton, tooltip3, null, null, null) || (KeyBindingDefOf.OpenMapSearch.JustPressed && Event.current.type == EventType.KeyDown))
		{
			Event.current.Use();
