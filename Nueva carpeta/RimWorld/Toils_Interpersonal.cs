...
public static class Toils_Interpersonal
{
	public static Toil GotoInteractablePosition(TargetIndex target)
	{
		...
		toil.tickIntervalAction = delegate
		{
			...
			Map map = actor2.Map;
			if (SocialInteractionUtility.IsGoodPositionForInteraction(actor2, pawn2) && actor2.Position.InHorDistOf(pawn2.Position, Mathf.CeilToInt(3f)) && (!actor2.pather.Moving || actor2.pather.nextCell.GetDoor(map) == null))
			{
			...
			{
				IntVec3 intVec = SocialInteractionUtility.BestInteractableCell(actor2, pawn2);
				if (intVec.IsValid)
				{
	...
	public static Toil ExtractHemogen(TargetIndex prisonerInd, float bloodLoss)
	{
		...
		toil.initAction = delegate
		{
			GenPlace.TryPlaceThing(ThingMaker.MakeThing(ThingDefOf.HemogenPack), toil.actor.Position, toil.actor.Map, ThingPlaceMode.Near, null, null, null);
