...
public static class DropCellFinder
{
	...
	{
		return CellFinderLoose.RandomCellWith((IntVec3 c) => (!standableOnly || c.Standable(map)) && !c.GetTerrain(map).IsWater && !c.Roofed(map) && !c.Fogged(map), map);
	}
	...
	public static IntVec3 TradeDropSpot(Map map)
	{
		...
		if (building != null)
		{
			...
			if (!TryFindDropSpotNear(position, map, out var result, allowFogged: false, canRoofPunch: false, allowIndoors: true, null))
			{
	...
	{
		if (!IsSafeDropSpot(c, map, faction, size, 5, 35, 15, null))
		{
			return false;
		}
	...
	public static IntVec3 GetBestShuttleLandingSpot(Map map, Faction factionForFindingSpot, out Thing firstBlockingThing)
	{
		...
		if (!result.IsValid && !FindSafeLandingSpot(out result, factionForFindingSpot, map, 35, 15, 25, ThingDefOf.Shuttle.Size + new IntVec2(2, 2), null))
		{
	...
	public static bool TryFindDropSpotNear(IntVec3 center, Map map, out IntVec3 result, bool allowFogged, bool canRoofPunch, int maxRadius, bool allowIndoors = true, IntVec2? size = null, bool mustBeReachableFromCenter = true)
	{
		...
		bool TryFindCell(Predicate<IntVec3> v, out IntVec3 r)
		{
			...
			do
			{
				if (CellFinder.TryFindRandomCellNear(center, map, num3, v, out r))
				{
					return true;
				}
				num3 += num2;
		...
		do
		{
			...
			else
			{
				if (parentFaction == Faction.OfPlayer)
				{
					...
					for (int i = 0; i < allBuildingsColonist.Count && !TryFindDropSpotNear(allBuildingsColonist[i].Position, map, out result, allowFogged: true, canRoofPunch, allowIndoors, null); i++)
					{
					}
				}
				else
				{
					...
					for (int j = 0; j < list.Count && (list[j].Faction != parentFaction || !TryFindDropSpotNear(list[j].Position, map, out result, allowFogged: true, canRoofPunch, allowIndoors, null)); j++)
					{
					}
				}
				if (!result.IsValid)
	...
	{
		return RCellFinder.TryFindRandomSpotNearAvoidingHostilePawns(thing, map, (IntVec3 s) => IsSafeDropSpot(s, map, thing.Faction, size, distToEdge, distToHostiles, distToFires, interactionCell), out spot);
	}
