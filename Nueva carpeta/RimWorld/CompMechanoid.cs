...
public class CompMechanoid : CompCanBeDormant
{
	...
	public override void ToSleep()
	{
		...
		Pawn?.GetLord()?.Notify_PawnLost(Pawn, PawnLostCondition.Incapped, null);
		Building?.GetLord()?.Notify_BuildingLost(Building, null);
		active = false;
	}
	...
	public override void PostSwapMap()
	{
		if (chargerBeforeTakeoff != null)
		{
			...
			Pawn.jobs.StartJob(newJob, JobCondition.InterruptForced, null, resumeCurJobAfterwards: false, cancelBusyStances: true, null, null, fromQueue: false, canReturnCurJobToPool: false, null);
