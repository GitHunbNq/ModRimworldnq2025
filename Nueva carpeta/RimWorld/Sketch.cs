...
public class Sketch : IExposable
{
	...
	public IEnumerable<IntVec3> GetSuggestedRoofCells()
	{
		...
		foreach (IntVec3 item in OccupiedRect)
		{
			...
			{
				if (occupiedRect.IsOnEdge(tmpSuggestedRoofCells[i]))
				{
					flag = true;
					break;
				}
			...
			for (int j = 0; j < tmpSuggestedRoofCells.Count; j++)
			{
				for (int k = 0; k < 9; k++)
				{
					IntVec3 intVec = tmpSuggestedRoofCells[j] + GenAdj.AdjacentCellsAndInside[k];
					if (!tmpYieldedSuggestedRoofCells.Contains(intVec) && occupiedRect.Contains(intVec) && (k == 8 || AnyRoofHolderAt(intVec)))
					{
	...
	public void AddPrefab(PrefabDef prefab, IntVec3 pos, Rot4 rot)
	{
		...
		foreach (var (prefabThingData, cell) in prefab.GetThings())
		{
			if (Rand.Chance(prefabThingData.chance))
			{
				...
				AddThing(prefabThingData.def, root + adjustedThingLocalPosition, rot.Rotated(prefabThingData.relativeRotation), prefabThingData.stuff, prefabThingData.stackCountRange.RandomInRange, null, null);
			}
		}
