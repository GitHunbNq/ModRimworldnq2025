...
public class Designator_Dropdown : DesignatorWithEyedropper
{
	...
	private Window SetupGridMenu(Event ev)
	{
		...
		for (int j = 0; j < elements.Count; j++)
		{
			...
			if (designator2 is Designator_Place { PlacingDef: { } placingDef3 } designator_Place3)
			{
				if (placingDef3.designatorDropdown.iconSource == DesignatorDropdownGroupDef.IconSource.Cost)
				{
					ThingDef designatorCost = GetDesignatorCost(designator2);
					if (designatorCost != null)
					{
						Texture2D iconFor = Widgets.GetIconFor(designatorCost, null, null, null);
						Action designatorSelectAction = GetDesignatorSelectAction(ev, designator2);
						TipSignal? tooltip = designator2.LabelCap;
						list.Add(new FloatMenuGridOption(iconFor, designatorSelectAction, null, tooltip));
						continue;
					}
				}
				if (placingDef3.designatorDropdown.iconSource == DesignatorDropdownGroupDef.IconSource.Placed)
				{
