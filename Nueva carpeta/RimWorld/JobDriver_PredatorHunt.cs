...
public class JobDriver_PredatorHunt : JobDriver
{
	...
	protected override IEnumerable<Toil> MakeNewToils()
	{
		...
		{
			Pawn prey2 = Prey;
			bool surpriseAttack = firstHit && !prey2.IsColonist;
			if (pawn.meleeVerbs.TryMeleeAttack(prey2, job.verbToUse, surpriseAttack))
			{
				if (!notifiedPlayerAttacked && PawnUtility.ShouldSendNotificationAbout(prey2))
				{
					notifiedPlayerAttacked = true;
					Messages.Message("MessageAttackedByPredator".Translate(prey2.LabelShort, pawn.LabelIndefinite(), prey2.Named("PREY"), pawn.Named("PREDATOR")).CapitalizeFirst(), prey2, MessageTypeDefOf.ThreatSmall);
				}
				base.Map.attackTargetsCache.UpdateTarget(pawn);
				firstHit = false;
			}
