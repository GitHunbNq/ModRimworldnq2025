...
public class MainTabWindow_History : MainTabWindow
{
	...
	private void DoArchivableRow(Rect rect, IArchivable archivable, int index)
	{
		...
		if (flag)
		{
			Widgets.DrawTextHighlight(rect, 0f, null);
	...
	private void DoGraphPage(Rect rect)
	{
		...
		if (Widgets.ButtonText(new Rect(legendRect.xMin + legendRect.width, legendRect.yMin, 110f, 40f), "Last30Days".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			graphSection = new FloatRange(Mathf.Max(0f, num - 30f), num);
			SoundDefOf.Click.PlayOneShotOnCamera();
		}
		if (Widgets.ButtonText(new Rect(legendRect.xMin + legendRect.width + 110f + 4f, legendRect.yMin, 110f, 40f), "Last100Days".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			graphSection = new FloatRange(Mathf.Max(0f, num - 100f), num);
			SoundDefOf.Click.PlayOneShotOnCamera();
		}
		if (Widgets.ButtonText(new Rect(legendRect.xMin + legendRect.width + 228f, legendRect.yMin, 110f, 40f), "Last300Days".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			graphSection = new FloatRange(Mathf.Max(0f, num - 300f), num);
			SoundDefOf.Click.PlayOneShotOnCamera();
		}
		if (Widgets.ButtonText(new Rect(legendRect.xMin + legendRect.width + 342f, legendRect.yMin, 110f, 40f), "AllDays".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			graphSection = new FloatRange(0f, num);
			SoundDefOf.Click.PlayOneShotOnCamera();
		}
		if (Widgets.ButtonText(new Rect(rect2.x, rect2.y, 110f, 40f), "SelectGraph".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
