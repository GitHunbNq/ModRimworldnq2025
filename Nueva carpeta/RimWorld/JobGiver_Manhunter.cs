...
public class JobGiver_Manhunter : ThinkNode_JobGiver
{
	...
	protected override Job TryGiveJob(Pawn pawn)
	{
		...
		if (pawn2 != null)
		{
			using (PawnPath pawnPath = pawn.Map.pathFinder.FindPathNow(pawn.Position, pawn2.Position, TraverseParms.For(pawn, Danger.Deadly, canBashDoors ? TraverseMode.PassDoors : TraverseMode.NoPassClosedDoors), null))
			{
				if (!pawnPath.Found)
				{
					return null;
