...
public class ITab_Fishing : ITab
{
	...
	protected override void FillTab()
	{
		...
		case FishRepeatMode.RepeatCount:
			listing_Standard2.Label(string.Concat("RepeatCount".Translate() + " ", zone.repeatCount.ToString()), -1f, (TipSignal?)null);
			listing_Standard2.IntEntry(ref zone.repeatCount, ref repeatCountEditBuffer);
			break;
		case FishRepeatMode.TargetCount:
		{
			listing_Standard2.Label(string.Concat("CurrentlyHave".Translate() + ": ", zone.OwnedFishCount.ToString(), " / ", zone.targetCount.ToString()), -1f, (TipSignal?)null);
	...
	private void ListFish(ThingDef fish, Listing_Standard listing, bool uncommon)
	{
		...
		Widgets.ThingIcon(rect3, fish, null, null, 1f, null, null);
