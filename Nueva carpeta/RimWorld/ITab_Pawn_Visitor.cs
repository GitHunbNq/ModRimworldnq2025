...
public abstract class ITab_Pawn_Visitor : ITab
{
	...
	private void DoSlaveTab(Listing_Standard listing)
	{
		...
		Rect rect6 = listing.Label(string.Format("{0}: {1}", "Terror".Translate(), SelPawn.GetStatValue(StatDefOf.Terror).ToStringPercent()), -1f, (TipSignal?)null);
	...
	private void DoPrisonerTab(bool wildMan, Listing_Standard listing)
	{
		...
		Rect rect = listing.Label(text, -1f, (TipSignal?)null);
