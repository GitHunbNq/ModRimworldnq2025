...
public abstract class QuestPart_SubquestGenerator : QuestPartActivable
{
	...
	private int PendingSubquestCount => quest.GetSubquests(null).Count((Quest q) => q.State == QuestState.Ongoing || q.State == QuestState.NotYetAccepted);
	...
	public override void DoDebugWindowContents(Rect innerRect, ref float curY)
	{
		...
		if (Widgets.ButtonText(rect, "Generate random subquest", drawBackground: true, doMouseoverSound: true, active: true, null) && CanGenerateSubquest)
		{
			TryGenerateSubquest();
		}
		curY += rect.height + 4f;
		Rect rect2 = new Rect(innerRect.x, curY, 500f, 25f);
		if (Widgets.ButtonText(rect2, "Generate specific subquest", drawBackground: true, doMouseoverSound: true, active: true, null))
		{
		...
		if (Widgets.ButtonText(rect3, "Remove active subquests", drawBackground: true, doMouseoverSound: true, active: true, null))
		{
		...
		if (Widgets.ButtonText(rect4, "Complete quest", drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			Complete();
		}
