...
public class Dialog_MapSearch : Dialog_Search<Thing>
{
	...
	{
		Widgets.ThingIcon(iconRect, element, 1f, null);
	}
	...
	protected override void ClikedOnElement(Thing element)
	{
		...
		{
			if (parentHolder.ParentHolder is Pawn pawn)
			{
				obj = pawn;
			}
	...
	private List<Thing> ContentsFromThing(Thing thing)
	{
		...
		if (thing is Pawn pawn && (pawn.IsColonist || pawn.IsPrisonerOfColony || pawn.IsAnimal))
		{
