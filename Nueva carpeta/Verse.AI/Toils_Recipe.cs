...
public static class Toils_Recipe
{
	...
	public static Toil DoRecipeWork()
	{
		...
		toil.tickAction = delegate
		{
			...
			Thing thing2 = actor2.jobs.curJob.GetTarget(TargetIndex.B).Thing;
			if (thing2 is UnfinishedThing && thing2.Destroyed)
			{
		...
		toil.tickIntervalAction = delegate(int delta)
		{
			...
			Job curJob2 = actor3.jobs.curJob;
			...
			{
				jobDriver_DoBill2.ticksSpentDoingRecipeWork += delta;
				curJob2.bill.Notify_PawnDidWork(actor3);
				if (curJob2.RecipeDef.workSkill != null && curJob2.RecipeDef.UsesUnfinishedThing && actor3.skills != null)
				{
					actor3.skills.Learn(curJob2.RecipeDef.workSkill, 0.1f * curJob2.RecipeDef.workSkillLearnFactor * (float)delta);
				}
				float num = ((curJob2.RecipeDef.workSpeedStat == null) ? 1f : actor3.GetStatValue(curJob2.RecipeDef.workSpeedStat));
				if (curJob2.RecipeDef.workTableSpeedStat != null && jobDriver_DoBill2.BillGiver is Building_WorkTable thing3)
				{
					num *= thing3.GetStatValue(curJob2.RecipeDef.workTableSpeedStat);
				}
				if (DebugSettings.fastCrafting)
				{
					num *= 30f;
				}
				jobDriver_DoBill2.workLeft -= num * (float)delta;
				...
				actor3.GainComfortFromCellIfPossible(delta, chairsOnly: true);
				if (jobDriver_DoBill2.workLeft <= 0f)
				{
					curJob2.bill.Notify_BillWorkFinished(actor3);
					jobDriver_DoBill2.ReadyForNextToil();
				}
				else if (curJob2.bill.recipe.UsesUnfinishedThing && Find.TickManager.TicksGame - jobDriver_DoBill2.billStartTick >= 3000 && actor3.IsHashIntervalTick(1000, delta))
				{
					actor3.jobs.CheckForJobOverride();
				}
			}
		};
		...
		toil.WithProgressBar(TargetIndex.A, delegate
		{
			...
			Job curJob3 = actor4.CurJob;
			Thing thing4 = curJob3.GetTarget(TargetIndex.B).Thing;
			float workLeft = ((JobDriver_DoBill)actor4.jobs.curDriver).workLeft;
			float num2 = ((curJob3.bill is Bill_Mech { State: FormingState.Formed }) ? 300f : curJob3.bill.recipe.WorkAmountTotal(thing4));
			return 1f - workLeft / num2;
		});
	...
	public static Toil FinishRecipeAndStartStoringProduct(TargetIndex productIndex = TargetIndex.A)
	{
		...
		toil.initAction = delegate
		{
			...
			if (curJob?.bill == null)
			{
				for (int i = 0; i < list.Count; i++)
				{
					if (!GenPlace.TryPlaceThing(list[i], actor.Position, actor.Map, ThingPlaceMode.Near, null, null, null))
					{
			...
			else
			{
				...
				else if (curJob.bill.GetStoreMode() == BillStoreModeDefOf.DropOnFloor)
				{
					...
					{
						if (!GenPlace.TryPlaceThing(list[j], actor.Position, actor.Map, ThingPlaceMode.Near, null, null, null))
						{
				...
				else
				{
					if (list.Count > 1)
					{
						...
						{
							if (!GenPlace.TryPlaceThing(list[k], actor.Position, actor.Map, ThingPlaceMode.Near, null, null, null))
							{
					...
					if (foundCell.IsValid)
					{
						...
						if (num < list[0].stackCount)
						{
							...
							if (!GenPlace.TryPlaceThing(thing2, actor.Position, actor.Map, ThingPlaceMode.Near, null, null, null))
							{
					...
					{
						if (!GenPlace.TryPlaceThing(list[0], actor.Position, actor.Map, ThingPlaceMode.Near, null, null, null))
						{
