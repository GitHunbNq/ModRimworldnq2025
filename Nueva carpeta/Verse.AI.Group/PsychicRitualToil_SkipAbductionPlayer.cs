...
public class PsychicRitualToil_SkipAbductionPlayer : PsychicRitualToil
{
	...
	private void ApplyOutcome(PsychicRitual psychicRitual, Pawn invoker)
	{
		...
		bool flag = false;
		Pawn[] source = psychicRitual.Map.attackTargetsCache.TargetsHostileToColony.Where((IAttackTarget t) => t.Thing is Pawn pawn && pawn.RaceProps.Humanlike && !pawn.IsSubhuman && pawn.Faction != Faction.OfPlayer && !t.ThreatDisabled(invoker) && !pawn.IsOnHoldingPlatform).Cast<Pawn>().ToArray();
		Pawn pawn2 = null;
