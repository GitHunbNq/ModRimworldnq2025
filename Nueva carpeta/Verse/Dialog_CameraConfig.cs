...
public class Dialog_CameraConfig : Window
{
	...
	public override void DoWindowContents(Rect rect)
	{
		...
		if (ModsConfig.OdysseyActive)
		{
			...
			{
				Widgets.FloatRange(rect6, Gen.HashCombine(100, this), ref Config.gravshipOverrideSizeRange, 0f, 100f, "ZoomRangeGravshipCutscene", ToStringStyle.FloatOne, 1f, GameFont.Small, null);
				rect6.y += 36f;
			}
		}
		...
		if (Widgets.ButtonText(rect8, "Reset", drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			Reset();
		}
		rect8.x += rect7.width / 3f;
		if (Widgets.ButtonText(rect8, "Save", drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			Find.WindowStack.Add(new Dialog_CameraConfigList_Save(Config));
		}
		rect8.x += rect7.width / 3f;
		if (Widgets.ButtonText(rect8, "Load", drawBackground: true, doMouseoverSound: true, active: true, null))
		{
