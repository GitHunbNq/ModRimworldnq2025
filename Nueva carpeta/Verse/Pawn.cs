		...
		try
		{
			...
			if (!PawnGenerator.IsPawnBeingGeneratedAndNotAllowsDead(this) && RaceProps.corpseDef != null)
			{
				...
				else if (spawnedOrAnyParentSpawned)
				{
					...
					corpse = MakeCorpse(assignedGrave, currentBed);
					if (GenPlace.TryPlaceThing(corpse, positionHeld, map2, ThingPlaceMode.Direct, null, null, null) || GenPlace.TryPlaceThing(corpse, positionHeld, map2, ThingPlaceMode.Near, null, null, null))
					{
