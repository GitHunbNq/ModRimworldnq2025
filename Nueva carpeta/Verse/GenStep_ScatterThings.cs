...
public class GenStep_ScatterThings : GenStep_Scatterer
{
	...
	protected override void ScatterAt(IntVec3 loc, Map map, GenStepParams parms, int stackCount = 1)
	{
		...
		if (thing.def.category == ThingCategory.Item)
		{
			...
			GenPlace.TryPlaceThing(thing, loc, map, ThingPlaceMode.Near, out var lastResultingThing, null, null, null);
	...
	protected override bool CanScatterAt(IntVec3 loc, Map map)
	{
		...
		if (!GenSpawn.CanSpawnAt(thingDef, loc, map, null))
		{
			return false;
		}
	...
	private bool IsRotationValid(IntVec3 loc, Rot4 rot, Map map)
	{
		...
		{
			IntVec3 c2 = loc + thingDef.interactionCellOffset.RotatedBy(rot);
		...
		if (skipRoofed && cellRect.Any((IntVec3 c) => c.Roofed(map)))
		{
			return false;
		}
