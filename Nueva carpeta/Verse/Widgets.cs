...
public static class Widgets
{
	...
	public static void ThingIcon(Rect rect, Thing thing, float alpha = 1f, Rot4? rot = null, bool stackOfOne = false, float scale = 1f, bool grayscale = false)
	{
		...
		{
			iconFor = GetIconFor(frame.BuildDef, frame.Stuff, frame.StyleDef, null);
		}
	...
	public static Texture2D GetIconFor(ThingDef thingDef, out Material material, ThingDef stuffDef = null, ThingStyleDef thingStyleDef = null, int? graphicIndexOverride = null)
	{
		...
		{
			result = ((!graphicIndexOverride.HasValue) ? thingStyleDef.UIIcon : thingStyleDef.IconForIndex(graphicIndexOverride.Value, null));
		}
	...
	public static Texture GetIconFor(Thing thing, Vector2 size, Rot4? rot, bool stackOfOne, out float scale, out float angle, out Vector2 iconProportions, out Color color, out Material material)
	{
		...
		else if (thing is Pawn pawn)
		{
			...
			else
			{
				...
				result = PortraitsCache.Get(pawn, size2, value, default(Vector3), cameraZoom, supersample: true, compensateForUIScale: true, renderHeadgear: true, renderClothes: true, null, null, stylingStation: false, null);
			}
		}
		else
		{
	...
	public static void WidgetsOnGUI()
	{
		...
		if (dropdownPainting)
		{
			GenUI.DrawMouseAttachment(dropdownPainting_Icon, dropdownPainting_Text, 0f, default(Vector2), null, null, drawTextBackground: false, default(Color), null);
		}
	}
}
