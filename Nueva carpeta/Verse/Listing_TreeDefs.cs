...
public class Listing_TreeDefs : Listing_Tree
{
	...
	{
		if (node.HasNewButton && widgetRow.ButtonIcon(TexButton.NewItem, null, null, null, null))
		{
		...
		if (node.nodeType == EditTreeNodeType.ListRoot && widgetRow.ButtonIcon(TexButton.Add, null, null, null, null))
		{
		...
		if (node.HasDeleteButton && widgetRow.ButtonIcon(TexButton.Delete, null, GenUI.SubtleMouseoverColor, null, null))
		{
			node.Delete();
		}
	}
	...
	protected void NodeLabelLeft(TreeNode_Editor node, int indentLevel)
	{
		...
		LabelLeft(node.LabelText, tipText, indentLevel, 0f, null);
	}
	...
	protected void ValueEditWidgetRight(TreeNode_Editor node, float leftX)
	{
		...
		{
			if (Widgets.ButtonText(rect, obj.ToString(), drawBackground: true, doMouseoverSound: true, active: true, null))
			{
