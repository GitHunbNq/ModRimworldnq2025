...
public static class DebugOutputsTechnical
{
	...
	public static void BestThingRequestGroup()
	{
		...
		{
			IEnumerable<ThingRequestGroup> source2 = (ListerThings.EverListable(d, ListerThingsUse.Global) ? ((ThingRequestGroup[])Enum.GetValues(typeof(ThingRequestGroup))).Where((ThingRequestGroup x) => x.Includes(d)) : Enumerable.Empty<ThingRequestGroup>());
			if (!source2.Any())
			{
				return "-";
			}
			ThingRequestGroup best2 = source2.MinBy((ThingRequestGroup x) => DefDatabase<ThingDef>.AllDefs.Count((ThingDef y) => ListerThings.EverListable(y, ListerThingsUse.Global) && x.Includes(y)));
			return best2.ToString() + " (defs: " + DefDatabase<ThingDef>.AllDefs.Count((ThingDef x) => ListerThings.EverListable(x, ListerThingsUse.Global) && best2.Includes(x)) + ")";
		}));
	}
