...
public class ThingDef : BuildableDef
{
	...
	public Thing GetConcreteExample(ThingDef stuff = null)
	{
		...
		if (!concreteExamplesInt.ContainsKey(stuff))
		{
			...
			{
				concreteExamplesInt[stuff] = PawnGenerator.GeneratePawn(DefDatabase<PawnKindDef>.AllDefsListForReading.FirstOrDefault((PawnKindDef pkd) => pkd.race == this), null, null);
			}
		}
		...
		if (race != null)
		{
			...
			if (race.body != null && race != null && tools != null)
			{
				...
				for (j = 0; j < tools.Count; j++)
				{
					...
					{
						yield return "has tool with linkedBodyPartsGroup " + tools[j].linkedBodyPartsGroup?.ToString() + " but body " + race.body?.ToString() + " has no parts with that group.";
					}
				}
			}
	...
	public override IEnumerable<StatDrawEntry> SpecialDisplayStats(StatRequest req)
	{
		...
		for (int j = 0; j < comps.Count; j++)
		{
			foreach (StatDrawEntry item7 in comps[j].SpecialDisplayStats(req))
			{
				yield return item7;
			}
		}
		...
		if (!equippedStatOffsets.NullOrEmpty())
		{
			for (int j = 0; j < equippedStatOffsets.Count; j++)
			{
				...
				float num8 = equippedStatOffsets[j].value;
				...
				yield return new StatDrawEntry(StatCategoryDefOf.EquippedStatOffsets, equippedStatOffsets[j].stat, num8, StatRequest.ForEmpty(), ToStringNumberSense.Offset, null, forceUnfinalizedMode: true).SetReportText(stringBuilder7.ToString());
			}
		}
		if (!IsDrug)
		{
