...
public class Gravship : WorldObject, IIncidentTarget, ILoadReferenceable
{
	...
	private void CheckAffectedGoodwill()
	{
		...
		foreach (KeyValuePair<Faction, int> item in tmpFactionMembersKidnappedCount)
		{
			...
			key.TryAffectGoodwillWith(goodwillChange: -10 * value, other: Faction.OfPlayer, canSendMessage: true, canSendHostilityLetter: true, reason: HistoryEventDefOf.PawnKidnappedOnGravShip, lookTarget: null);
		}
	}
