	...
	public static PlanetTile RandomSettlementTileFor(PlanetLayer layer, Faction faction, bool mustBeAutoChoosable = false, Predicate<PlanetTile> extraValidator = null)
	{
		for (int i = 0; i < 500; i++)
		{
				...
				select Rand.Range(0, layer.TilesCount)).TryRandomElementByWeight(delegate(int x)
			{
				...
				if (mustBeAutoChoosable && !tile.PrimaryBiome.canAutoChoose)
				{
					return 0f;
				}
				if (extraValidator != null && !extraValidator(tile.tile))
				{
					return 0f;
				}
				float num = tile.PrimaryBiome.settlementSelectionWeight;
				if (faction?.def.minSettlementTemperatureChanceCurve != null)
				{
					num *= faction.def.minSettlementTemperatureChanceCurve.Evaluate(GenTemperature.MinTemperatureAtTile(tile.tile));
				}
				return num;
			}, out var result))
			{
				PlanetTile tile2 = layer[result].tile;
				if (IsValidTileForNewSettlement(tile2))
				{
					return tile2;
				}
			}
