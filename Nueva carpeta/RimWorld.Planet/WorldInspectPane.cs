	...
	private string TileInspectString
	{
		get
		{
			...
			if (tile is SurfaceTile surfaceTile)
			{
				...
				{
					string arg = (WorldPathGrid.CalculatedMovementDifficultyAt(SelectedTile, perceivedStatic: false, null) * Find.WorldGrid.GetRoadMovementDifficultyMultiplier(SelectedTile, PlanetTile.Invalid)).ToString("0.#");
					stringBuilder.AppendLine();
