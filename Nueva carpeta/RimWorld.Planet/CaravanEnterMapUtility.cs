...
public static class CaravanEnterMapUtility
{
	...
	public static void Enter(Caravan caravan, Map map, Func<Pawn, IntVec3> spawnCellGetter, CaravanDropInventoryMode dropInventoryMode = CaravanDropInventoryMode.DoNotDrop, bool draftColonists = false)
	{
		...
		if (shuttle != null)
		{
			if (!CellFinder.TryFindRandomCellNear(tmpPawns.RandomElement().Position, map, 20, (IntVec3 c) => RoyalTitlePermitWorker_CallShuttle.ShuttleCanLandHere(c, map, null, null), out var result) && CellFinder.TryFindRandomCell(map, (IntVec3 c) => RoyalTitlePermitWorker_CallShuttle.ShuttleCanLandHere(c, map, null, null), out result))
			{
				result = tmpPawns.RandomElement().Position;
			}
			if (!GenPlace.TryPlaceThing(shuttle, result, map, ThingPlaceMode.Near, null, null, null))
			{
