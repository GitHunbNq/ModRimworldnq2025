...
public class Caravan : WorldObject, IThingHolder, IIncidentTarget, ILoadReferenceable, ITrader
{
	...
	public override IEnumerable<Gizmo> GetGizmos()
	{
		...
		{
			if (PawnsListForReading.Where((Pawn x) => x.needs.food != null).TryRandomElement(out var result2))
			{
				result2.needs.food.CurLevelPercentage = 0f;
			}
		};
		...
		command_Action5.action = delegate
		{
			...
			{
				result3.Kill(null);
				Messages.Message("DEV: Killed " + result3.LabelShort, this, MessageTypeDefOf.TaskCompletion, historical: false);
			}
		};
		...
		{
			if (PawnsListForReading.Where((Pawn x) => !x.Downed).TryRandomElement(out var result5))
			{
				HealthUtility.DamageUntilDowned(result5);
				Messages.Message("DEV: Downed " + result5.LabelShort, this, MessageTypeDefOf.TaskCompletion, historical: false);
			}
		};
		...
		{
			if (PawnsListForReading.Where((Pawn x) => !x.Downed && x.RaceProps.IsFlesh).TryRandomElement(out var result6))
			{
				Hediff hediff = HediffMaker.MakeHediff(HediffDefOf.Plague, result6);
				hediff.Severity = HediffDefOf.Plague.stages[1].minSeverity - 0.001f;
				result6.health.AddHediff(hediff, null, null);
				Messages.Message("DEV: Gave advanced plague to " + result6.LabelShort, this, MessageTypeDefOf.TaskCompletion, historical: false);
			}
		};
