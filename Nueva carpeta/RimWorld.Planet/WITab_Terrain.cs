...
public class WITab_Terrain : WITab
{
	...
	private void DrawScrollContents(Listing_Standard listing, Rect infoRect)
	{
		...
		listing.Label(selTile.PrimaryBiome.description, -1f, (TipSignal?)null);
		listing.Gap(8f);
		listing.GapLine();
		if (!selTile.PrimaryBiome.implemented)
		{
			listing.Label(string.Format("{0} {1}", selTile.PrimaryBiome.LabelCap, "BiomeNotImplemented".Translate()), -1f, (TipSignal?)null);
		}
	...
	private static void ListGeometricDetails(Listing_Standard listing, Tile ws, PlanetTile tile)
	{
		...
		if (!Find.World.Impassable(tile))
		{
			...
			if (WorldPathGrid.WillWinterEverAffectMovementDifficulty(tile) && WorldPathGrid.GetCurrentWinterMovementDifficultyOffset(tile, null) < 2f)
			{
