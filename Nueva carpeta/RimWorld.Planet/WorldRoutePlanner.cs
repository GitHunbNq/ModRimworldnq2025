...
public class WorldRoutePlanner
{
	...
	private bool DoChooseRouteButton()
	{
		...
		if (Widgets.ButtonText(new Rect(((float)UI.screenWidth - BottomButtonSize.x) / 2f, (float)UI.screenHeight - BottomWindowSize.y - 45f - 10f - BottomButtonSize.y, BottomButtonSize.x, BottomButtonSize.y), "Accept".Translate(), drawBackground: true, doMouseoverSound: true, active: true, null))
		{
			Find.WindowStack.Add(currentFormCaravanDialog);
